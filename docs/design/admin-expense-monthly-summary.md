# 設計書: 管理画面「経費管理」— 作業員×期の経費一覧（確認の土台）

> 注（更新）: 当初は「作業員×月（前半・後半を合算）で1行」としたが、申請/差し戻し/締切が
> すべて半月(期)単位になったため、**一覧は「作業員×期(前半/後半)」で別行**に変更した
> （batch `expense-application-flow` の C で実装）。月ナビゲーションは維持。
