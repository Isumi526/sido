...
test('AC1: 見積書をPDF付きで登録 → 一覧に業者・現場・PDFが出る', async ({ page }) => {
  ...
  const ests = await restSrv(`estimates?note=eq.${encodeURIComponent(TOKEN)}&select=id,estimate_number,total_amount,subcontractor_id,site_id,pdf_path,construction_details&order=estimate_number`)
...
test('AC3: 見積番号は EST-<年>-<連番>。2件目は連番が増える', async ({ page }) => {
  ...
  const ests = await restSrv(`estimates?note=eq.${encodeURIComponent(TOKEN)}&select=estimate_number&order=estimate_number`)
