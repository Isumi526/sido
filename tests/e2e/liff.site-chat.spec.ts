...
test('自分あての未読メンションがHOME/ハンバーガーの現場情報ナビにバッジ表示され、チャットを開くと既読になる', async ({ page }) => {
  const accountId = await getAccountId()
  ...
  try {
    ...
    await expect(page.getByTestId('home-mention-badge')).toBeVisible({ timeout: 10000 })

    await page.goto(`/site-chat/${siteAId}`, { waitUntil: 'networkidle' })
    await page.goto('/', { waitUntil: 'networkidle' })
    await expect(page.getByTestId('home-mention-badge')).toHaveCount(0, { timeout: 10000 })
  } finally {
