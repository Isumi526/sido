...
test('LINE風の吹き出し: 自分のメッセージはアバター無し+右しっぽ、相手のメッセージはアバター有り+左しっぽ(2026-07-20)', async ({ page }) => {
  const accountId = await getAccountId()
  const otherMsgId = (await restSrv('site_chat_messages', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    account_id: accountId, site_id: siteAId, sender_is_admin: true, sender_name: 'テスト管理者', body: `相手からのメッセージ_${TS}`,
  }) }))[0].id
  try {
    await page.goto(`/site-chat/${siteAId}`, { waitUntil: 'networkidle' })
    const myRow = page.locator('.msg-row', { hasText: `E2Eテストメッセージ_${TS}` })
    await expect(myRow).toHaveClass(/mine/)
    await expect(myRow.locator('.msg-avatar')).toHaveCount(0)
    await expect(myRow.locator('.msg-tail-right')).toBeVisible()

    const otherRow = page.locator('.msg-row', { hasText: `相手からのメッセージ_${TS}` })
    await expect(otherRow).not.toHaveClass(/mine/)
    await expect(otherRow.locator('.msg-avatar')).toBeVisible()
    await expect(otherRow.locator('.msg-tail-left')).toBeVisible()
  } finally {
    await restSrv(`site_chat_messages?id=eq.${otherMsgId}`, { method: 'DELETE' }).catch(() => {})
  }
})

test('日付区切りチップが表示され、タイムスタンプは吹き出しの外に表示される(2026-07-20)', async ({ page }) => {
  const accountId = await getAccountId()
  const yesterday = yesterdayNoonJST()
  const yestMsgId = (await restSrv('site_chat_messages', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    account_id: accountId, site_id: siteAId, sender_is_admin: true, sender_name: 'テスト管理者', body: `昨日のメッセージ_${TS}`, created_at: yesterday,
  }) }))[0].id
  try {
    await page.goto(`/site-chat/${siteAId}`, { waitUntil: 'networkidle' })
    await expect(page.locator('.day-sep-label', { hasText: '昨日' })).toBeVisible({ timeout: 10000 })
    await expect(page.locator('.day-sep-label', { hasText: '今日' })).toBeVisible()

    const myRow = page.locator('.msg-row', { hasText: `E2Eテストメッセージ_${TS}` })
    await expect(myRow.locator('.msg-bubble .msg-time-outside')).toHaveCount(0)
    await expect(myRow.locator('.msg-time-outside')).toBeVisible()
  } finally {
    await restSrv(`site_chat_messages?id=eq.${yestMsgId}`, { method: 'DELETE' }).catch(() => {})
  }
})
...
test('@allを選択して送信すると、その時点のaccount内の全アクティブworker(自分を除く)宛にメンション通知が作られる', async ({ page }) => {
  const accountId = await getAccountId()
  const activeWorkers = await rest(`workers?account_id=eq.${accountId}&active=eq.true&select=id`)
  const users = await rest('users?line_user_id=eq.dev-user-id&select=worker_id')
  const myWorkerId = users[0].worker_id
  // site-chat/[id].vueのsend()は自分自身へのメンション行は作らない(mentionIds.filter(id!==myWorkerId))ため、
  // @allで展開されたIDにも同じフィルタが効き、期待件数は「自分を除く」アクティブworker数になる。
  const expectedCount = activeWorkers.filter((w: { id: string }) => w.id !== myWorkerId).length
  await page.goto(`/site-chat/${siteAId}`, { waitUntil: 'networkidle' })
