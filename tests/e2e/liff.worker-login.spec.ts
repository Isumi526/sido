...
test('AC: users行が無い email/pw作業員もログインで users行が作成される（日報がuser_id付きで保存される前提）', async ({ page }) => {
  ...
  await restSrv(`users?worker_id=eq.${wid}`, { method: 'DELETE' }).catch(() => {})
...
test('クエリパラメータでメール/パスワードが事前入力される（毎回入力不要）', async ({ page }) => {
  await page.goto('/login?email=demo1@email.com&pass=demo1234')
  await expect(page.getByTestId('login-email')).toHaveValue('demo1@email.com', { timeout: 10000 })
