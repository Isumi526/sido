...
test('全体チャット(admin)でもファイル選択でpending表示になる(2026-07-20)', async ({ page }) => {
  await page.goto('/chats/account', { waitUntil: 'networkidle' })
...
test('全体チャット(admin)に保存済みの添付付きメッセージが正しく表示される(2026-07-20)', async ({ page }) => {
  const accountId = await getAccountId()
  const fileMsgId = (await restSrv('site_chat_messages', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    ...
    body: '', attachment_url: 'https://example.com/fake-account.pdf', attachment_name: 'doc-account.pdf', attachment_kind: 'file',
  }) }))[0].id
  try {
    await page.goto('/chats/account', { waitUntil: 'networkidle' })
    const fileLink = page.locator('.msg-attachment-file', { hasText: 'doc-account.pdf' })
    await expect(fileLink).toBeVisible({ timeout: 10000 })
    await expect(fileLink).toHaveAttribute('href', 'https://example.com/fake-account.pdf')
  } finally {
