...
//  シード現場が出ることを検証する。
//  2026-07-15: 明細の「対象」列に経費の申請者名(日報送信者)も表示されるようになった
//  （従来は現場名のみで「誰の経費か」がダッシュボードから分からなかった）。
// ============================================================
...
test.beforeAll(async () => {
  ...
  const u = await rest(`users?account_id=eq.${accountId}&line_user_id=eq.dev-user-id&select=id,real_name,workers(name)`)
  const devUserId = u[0].id
  submitterName = u[0].workers?.name ?? u[0].real_name ?? '—'
...
test('月次集計の項目クリックで当月明細が表示される（軽油代→シード現場＋申請者名）', async ({ page }) => {
