...
test('日付軸の先頭ティックがsticky corner配下に隠れず、translateXオフセットが掛からない(2026-07-20回帰防止)', async ({ page }) => {
  ...
  // 2件目以降のティック(存在すれば)は従来どおり-2pxオフセットが掛かっていることも確認
  const otherTicks = page.locator('.gantt-tick:not(.gantt-tick-first)')
  if (await otherTicks.count()) {
    const transform = await otherTicks.first().evaluate(el => getComputedStyle(el).transform)
