...
test('長期タスクが混ざっても短期タスクの帯が視認できるよう、表示軸をクランプし長期タスクは打ち切り表示になる', async ({ page }) => {
  ...
  try {
    ...
    // 長期タスクの帯には終端側の打ち切りクラスが付く
    const longGroup = page.locator('.gantt-group', { hasText: longSite })
    await expect(longGroup.locator('.gantt-bar.gantt-bar-truncated-end')).toBeVisible()
...
test('長期タスクの開始日が通常タスクより早くても、表示窓は今日周辺に留まり通常タスクの帯が潰れない', async ({ page }) => {
  // 表示窓を「全タスクの最早日」で固定すると、この長期タスク(今日より遥か前に開始)が起点になり、
  // 今日周辺の通常タスクが窓の外に押し出されて幅1.5%まで潰れる不具合があった(2026-07-16発見・修正)。
  const accountId = await getAccountId()
  const earlySite = `E2E早期開始長期現場_${TS}`
  const earlyTaskName = `E2E遥か前開始の長期改修_${TS}`
  const nearSite = `E2E通常現場_${TS}`
  const nearTaskName = `E2E通常工事_${TS}`
  ...
  const nearSiteId = (await rest('sites', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    account_id: accountId, name: nearSite, active: true,
  }) }))[0].id
  // 長期タスクは60日前(取得窓WINDOW_DAYS_PASTの境界内)に開始し500日続く＝開始日が全タスク中で最も早い。
  const earlyTaskId = (await restSrv('process_tasks', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    ...
    start_date: isoPlus(-59), end_date: isoPlus(441), progress: 0,
  }) }))[0].id
  // 通常タスクは今日から2週間後の短期工事(長期タスクの開始からは大きく離れている)。
  const nearTaskId = (await restSrv('process_tasks', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify({
    account_id: accountId, site_id: nearSiteId, name: nearTaskName,
    start_date: isoPlus(14), end_date: isoPlus(18), progress: 0,
  }) }))[0].id
  try {
    await page.goto('/company-schedule', { waitUntil: 'networkidle' })
    const nearGroup = page.locator('.gantt-group', { hasText: nearSite })
    const nearBar = nearGroup.locator('.gantt-bar').first()
    await expect(nearBar).toBeVisible({ timeout: 15000 })
    // 通常タスクは打ち切り扱いされず、幅も最小幅(1.5%)まで潰れていないこと
    await expect(nearBar).not.toHaveClass(/gantt-bar-truncated/)
    const width = await nearBar.evaluate(el => parseFloat((el as HTMLElement).style.width))
    expect(width).toBeGreaterThan(1.5)
  } finally {
    await restSrv(`process_tasks?id=eq.${earlyTaskId}`, { method: 'DELETE' }).catch(() => {})
    await restSrv(`process_tasks?id=eq.${nearTaskId}`, { method: 'DELETE' }).catch(() => {})
