// ============================================================
//  liff.calendar-birthday-badge.spec.ts
//  作業員の生年月日(workers.birth_date)を登録すると、共有カレンダーの
...
const TS = Date.now()
const BIRTHDAY_WORKER = `E2E誕生日作業員_${TS}`
const OTHER_WORKER = `E2E誕生日無し作業員_${TS}`
...
test('今日が誕生日の作業員には共有カレンダーに誕生日バッジが出て、それ以外の作業員には出ない', async ({ page }) => {
  await page.goto('/calendar', { waitUntil: 'networkidle' })
  await page.locator('.cal-tab', { hasText: '共有' }).click()
