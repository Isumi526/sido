...
const [DATE_SNAP, DATE_A, DATE_B, DATE_CLIP] = pickDays(4)
...
test.beforeAll(async () => {
  ...
  // (3) シフト外休憩のクリップ: 昼勤08:00-16:00 だが 現場休憩は昼12:00/60＋夜22:30/30 の両方。
  //     22:30 はシフト外なので適用されず、12:00の60分だけ効く → 稼働 8h-1h=7h(全通常) = 7×2750 = ¥19,250。
  //     （もし両方=90分適用の誤りなら 6.5h=¥17,875 になる）
  await postReport(DATE_CLIP, SITE_CLIP, {
    workerName: WORKER, workerRole: 'site', startTime: '08:00', endTime: '16:00',
    breakMinutes: 90, breakSnapshot: true, breaks: [{ start: '12:00', minutes: 60 }, { start: '22:30', minutes: 30 }],
  })
})
...
test('シフト外の休憩は適用しない: 昼勤に夜勤用休憩(22:30)は効かず昼休憩60分だけ(¥19,250)', async ({ page }) => {
  await page.goto('/site-reports', { waitUntil: 'networkidle' })
  await expect(page.locator('h1')).toContainText('現場別集計')
  // 12:00/60(シフト内)のみ適用=7h=¥19,250。22:30/30(シフト外)まで引くと¥17,875になるので、そうでないこと。
  const txt = await siteLaborText(page, SITE_CLIP)
  expect(txt).toContain('¥19,250')
  expect(txt).not.toContain('¥17,875')
})
