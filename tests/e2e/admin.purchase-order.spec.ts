...
test.beforeAll(async () => {
  ...
  const subs  = await rest(`subcontractors?account_id=eq.${accountId}&active=eq.true&select=id,name&limit=1`)
  const sites = await rest(`sites?account_id=eq.${accountId}&active=eq.true&select=id,name&limit=1`)
  subId = subs[0].id
  siteName = sites[0].name
  const siteId = sites[0].id

  // 担当者（メール付き）を作成 → 注文書メールの宛先になる
  const c = await rest('subcontractor_contacts', {
    method: 'POST',
...
test.afterAll(async () => {
  // 後始末：注文書 → トークン → 見積 → 担当者
  if (orderId) {
    await rest(`document_access_tokens?document_id=eq.${orderId}`, { method: 'DELETE' }).catch(() => {})
    await rest(`purchase_orders?id=eq.${orderId}`, { method: 'DELETE' }).catch(() => {})
  }
  await rest(`purchase_orders?estimate_id=eq.${estimateId}`, { method: 'DELETE' }).catch(() => {})
  await rest(`estimates?note=eq.${encodeURIComponent(TOKEN)}`, { method: 'DELETE' }).catch(() => {})
  if (contactId) await rest(`subcontractor_contacts?id=eq.${contactId}`, { method: 'DELETE' }).catch(() => {})
})
...
test('AC1〜AC4: 見積書から注文書を発行 → 採番・PDF・承諾メール送信', async ({ page }) => {
  ...
  await expect(firstRow.locator('.mono')).toContainText(/PO-\d{4}-\d{4}/)   // AC1: 採番
  await expect(firstRow.locator('.badge.ok')).toHaveText('送信済み')         // AC4: メール送信済み
  await expect(firstRow.locator('.pdf-link')).toBeVisible()                  // AC3: PDF生成

  // DB 検証：注文書 + トークン（order_accept / purchase_order）が作られている
  const orders = await rest(`purchase_orders?estimate_id=eq.${estimateId}&select=id,order_number,total_amount,email_sent_at,pdf_path,vendor_contact_name,payment_terms`)
  ...
  const tokens = await rest(`document_access_tokens?document_id=eq.${orderId}&purpose=eq.order_accept&document_type=eq.purchase_order&select=id,token_hash`)
