...
8.5 先方送付セット（T44・2026-09-19・自動）: スモーク異常なしの後、**client-report skill** で
   - リリースノート（A4・箇条書き・`docs/releases/` の HTML/PDF に加えて **Notion お客様共有版**の子ページとして作る）
   - 反映した設計書の **突合レポート**（設計書DBの `突合レポート`・お客様共有版の公開URL）
   - **送付文面の下書き**（LINE/メール・コードブロック・CLAUDE.md「送信文面の関門」を通す）
   を揃え、/ball 区分「送付待ち」に出す。**送るのは人**（🚩G2・外部送信は自動化しない・第6条）。設計書の `送付待ち` は client-report が立て、人が送ったら消す。
