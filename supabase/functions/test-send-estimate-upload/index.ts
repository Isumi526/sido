// test-send-estimate-upload: トークン発行・記録は本番同様だが実送信しない（dryRun・verify_jwt=false・本番では動かさない）。
// キーは subcontractor_id（業者ごと再利用リンク）。mode: prepare/copy/send を転送。send でも dryRun=true で Resend を叩かない。
...
function isProd(){return /^https:\/\/[a-z0-9-]+\.supabase\.co/i.test(Deno.env.get('SUPABASE_URL')??'')}
Deno.serve(async(req)=>{
  ...
  const{status,body:resBody}=await sendEstimateUpload({subcontractor_id,mode:mode as any,subject,body,to,dryRun:true,callerAuth:req.headers.get('Authorization')})
