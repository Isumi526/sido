// ============================================================
//  supabase/functions/vehicle-shaken-ocr
//  車検証の画像から「有効期間の満了する日（＝次回車検の期日）」を AI(Gemini vision) で読取（#10）。
//   - 入力: { account_slug, image_base64, mime }
//   - 出力: { ok, inspection_date }（YYYY-MM-DD／読めなければ null）
//   - 解析結果は返すだけ＝自動でDBには書かない。admin 画面が日付欄へ prefill し、
...
const PROMPT = `添付画像は日本の自動車検査証（車検証）です。
「有効期間の満了する日」（次回車検の期日）を読み取り、JSON のみで出力してください。
出力形式: {"inspection_date": "YYYY-MM-DD"}（例: {"inspection_date": "2027-03-15"}）
ルール:
- 和暦（令和など）が書かれていれば西暦に変換する（令和7年 = 2025年）。
- 「有効期間の満了する日」が見つからない/判読不能なら {"inspection_date": null}。
- 憶測で埋めない。確実に読めた日付だけを出す。
- 必ず YYYY-MM-DD のゼロ埋め形式にする。`

async function geminiShakenDate(imageB64: string, mime: string): Promise<string | null> {
  ...
  try {
    ...
    const d = (obj?.inspection_date ?? '').toString().trim()
    // YYYY-MM-DD の妥当性だけ軽く検証（不正なら null）
...
Deno.serve(async (req) => {
  ...
  try {
    const inspection_date = await geminiShakenDate(image_base64, mime || 'image/jpeg')
    return json({ ok: true, inspection_date })
  } catch (e) {
