...
async function applyApprovedToReport(svc: any, accountId: string, r: {
  ...
  requested_break_minutes: number | null; is_late: boolean | null; reported_end_time?: string | null
}): Promise<boolean> {
  try {
    ...
    // 書く内容が無ければ触らない
    if (bm === null && !endToApply && !(isLate && r.requested_start_time)) return false
    ...
    for (const rep of reports) {
      ...
      const n = applyApprovedOvertimeToSites(sites, r.worker_id, {
        breakMinutes: bm, startTime: isLate ? toHm(r.requested_start_time) : null, endTime: endToApply, isLate,
      })
      if (!n) continue
...
Deno.serve(async (req) => {
  ...
  if (body.action === 'overtime-decide') {
    ...
    const { data: reqRow } = await svc.from('overtime_requests')
      .select('id, worker_id, status, date, requested_start_time, requested_end_time, requested_break_minutes, is_late, reported_end_time')
  ...
  if (body.action === 'overtime-status') {
    ...
    const { data } = await svc.from('overtime_requests')
      .select('status, is_late, requested_start_time, requested_end_time, requested_break_minutes, reason, site_names, requested_at, decision_note, reported_end_time')
  ...
  if (body.action === 'overtime-report-end') {
    const date = isDate(body.date) ? body.date : ''
    if (!date) return json({ ok: false, error: 'bad_date' }, 400)
    const endTime = body.endTime === null ? null : (isTime(body.endTime) ? String(body.endTime).slice(0, 5) : undefined)
    if (endTime === undefined) return json({ ok: false, error: 'bad_time' }, 400)
    const { data: rows } = await svc.from('overtime_requests').select('id, status')
      .eq('account_id', caller.accountId).eq('worker_id', caller.workerId).eq('date', date)
      .order('requested_at', { ascending: false }).limit(1)
    const r = (rows ?? [])[0] as any
    if (!r || r.status !== 'pending') {
      if (endTime === null) return json({ ok: true, changed: 0 })   // 消すものが無い＝成功扱い
    ...
    const { data: upd, error } = await svc.from('overtime_requests')
      .update({ reported_end_time: endTime }).eq('id', r.id).eq('status', 'pending').select('id')
    if (error) {
      console.error('[attendance-log] overtime-report-end failed:', error)
      return json({ ok: false, error: 'update_failed' }, 500)
    }
