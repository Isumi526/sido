// ============================================================
//  worker-attachment-url
//  作業員の添付（履歴書等・非公開バケット worker-attachments）の短TTL署名URLを発行する。
//   - 入力: { attachment_id }
//   - 認可: Authorization JWT（admin / office）→ app_metadata.account_slug → account。
//     添付の account と一致する時のみ発行。不一致=403 / 認可不可=401。
//   - service_role で worker_attachments を引き、createSignedUrl(短TTL) を返す。
//  ※ verify_jwt=false（関数内で Supabase JWT を厳密検証）。CIが全関数を --no-verify-jwt でデプロイ。
// ============================================================
...
const BUCKET = 'worker-attachments'
...
// caller の account_id を JWT（admin / office）から解決
...
Deno.serve(async (req) => {
  ...
  const { data: att, error: attErr } = await svc
    .from('worker_attachments').select('id, account_id, path').eq('id', attachment_id).maybeSingle()
