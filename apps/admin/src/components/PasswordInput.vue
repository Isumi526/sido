<!-- パスワード入力＋表示/非表示トグル。既存の <input type="password"> を差し替えて使う。
     ★スタイル方針: 親から渡される class（例 "input"）は "枠" である wrap(コンポーネントroot)に付け、
       ...
       input に届かない問題を回避）。data-testid/autocomplete/placeholder/name 等は input へ委譲。 -->
...
<style scoped>
/* wrap が親の class(例 .input) を受けて "枠" になる。flex で input と目玉を横並び。 */
.pw-field { display: flex; align-items: center; gap: 4px; }
/* 中の input は透明＝枠のスタイル(bg/border/radius/padding)を wrap 側に任せ、font/color は継承。 */
.pw-input {
