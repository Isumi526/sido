<template>
  <div>
    <div class="page-header">
      <div>
        <h1 class="page-title">備品・カード（ETC など）</h1>
      </div>
      <button class="btn-add" data-testid="asset-generate-open" @click="openGenerate">＋ 枚数を指定して追加</button>
    </div>
    <p class="page-note">
      ETCカードなど「何があるか」だけを持つ備品の一覧です（数量は持ちません）。
      資材の残数を数えるものは「資材の在庫」、QRを貼って持出／返却を追う1点物は「道具」に入れてください。枚数を指定すると「丸一1」「丸一2」…と連番でデフォルト名が付きます。名前は後から自由に変更できます。
