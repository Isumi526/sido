<template>
  <div>
    <div class="page-header">
      <h1 class="page-title">工程管理
        <HelpButton title="工程管理の使い方" :items="[
          '「月ビュー」は全現場の工期（現場マスタで入力）を月単位で俯瞰します。現場は住所の地方ごとにまとまり、行のクリップから工程表PDFを開けます。',
          '工期が未入力の現場は「工期未定」に出ます。現場マスタの編集で工期を入れると帯が出ます。',
          '「工程（詳細）」はExcel取込などで登録した工程（タスク）を日単位のガントで表示します。月ビューの現場名を押すとその現場の工程に切り替わります。',
          '「＋ 工程を追加」から工程を登録。各行の編集/削除で更新できます。',
        ]" />
      </h1>
      <div class="header-actions">
        <div class="view-toggle" role="tablist">
          <button type="button" class="vt" :class="{ on: viewMode === 'month' }" data-testid="view-month" @click="viewMode = 'month'">月ビュー</button>
          <button type="button" class="vt" :class="{ on: viewMode === 'tasks' }" data-testid="view-tasks" @click="viewMode = 'tasks'">工程（詳細）</button>
        </div>
        <select v-model="siteId" class="input site-select" @change="onPickSite">
    ...
    <!-- 月ビュー（既定）: 行＝現場（責任者付き）・列＝月・帯＝現場マスタの工期・地方でグループ化。
         2026-09-10 SEED 大塚さん「いろんな現場が担当者を持って…1・2・3月…パッと見てこの現場はこの辺だな」
         「詳細はいいから工期だけ分かればいい」「関東・中部・近畿で地域ごとにまとめて見たい」 -->
    <div v-if="viewMode === 'month'" class="month-wrap" data-testid="month-view">
      ...
      <div v-else class="mcal" :style="{ '--mlabel-w': MLABEL_W + 'px' }">
        ...
        <div v-for="g in monthGroupsBySite" :key="g.key" class="mcal-group" :data-testid="`region-${g.key}`">
          ...
          <div v-for="r in (collapsedGroups.has(g.key) ? [] : g.rows)" :key="r.site.id" class="m-row" :data-testid="`month-site-${r.site.id}`">
            <div class="m-label">
              <button type="button" class="m-site" :title="'この現場の工程（詳細）を開く'" @click="openSiteTasks(r.site.id)">{{ r.site.name }}</button>
              <span v-if="r.night" class="m-night" title="夜間帯の定時がある現場">夜</span>
              <button v-for="a in r.atts" :key="a.id" type="button" class="m-clip" :title="`工程表: ${a.name || 'PDF'}`" :data-testid="`clip-${a.id}`" @click="openSchedulePdf(a.id)">
    ...
    <div v-else-if="!siteId" class="empty">現場を選択してください。</div>
...
const viewMode = ref<'month' | 'tasks'>('month')
const MLABEL_W = 260
const MONTH_W = 112
...
function openSiteTasks(sid: string) { siteId.value = sid; viewMode.value = 'tasks'; load() }
// 現場プルダウンは工程（詳細）の操作なので、選んだら詳細ビューに切り替える（全現場も同様）
function onPickSite() { viewMode.value = 'tasks'; load() }
...
function openAdd() { viewMode.value = 'tasks'; openSiteEditor(isAll.value ? '' : siteId.value) }
...
.vt { background: #fff; border: 0; padding: 6px 12px; font-size: 13px; cursor: pointer; color: #475569; }
.vt.on { background: #06C755; color: #fff; font-weight: 700; }
...
.m-site { background: none; border: 0; padding: 0; font-size: 13px; font-weight: 700; color: #1e293b; cursor: pointer; text-align: left; }
.m-site:hover { color: #06C755; text-decoration: underline; }
