    ...
    <main class="wrap">
      ...
      <template v-else>
        <!-- 月ビュー（既定）: 行＝現場・列＝月・帯＝現場マスタの工期・地方でグループ化（2026-09-10 SEED 大塚さん）。
             作業員にも「どの現場がいつ頃か」が一画面で分かる。クリップから工程表PDFを開ける。 -->
        ...
        <!-- 工程（詳細・Excel取込などの工程行）は折りたたみ。2026-09-10「詳細はいいから工期だけ分かればいい」 -->
        <button type="button" class="detail-toggle" data-testid="tasks-toggle" @click="showTasks = !showTasks">
          <span class="material-symbols-rounded" style="font-size:18px;line-height:1;vertical-align:middle">{{ showTasks ? 'expand_less' : 'expand_more' }}</span>
          {{ $t('companySchedule.tasksToggle') }}<span class="detail-count">{{ items.length }}</span>
        </button>
        <div v-if="showTasks && !items.length" class="state">{{ $t('companySchedule.empty') }}</div>
      </template>

      <template v-if="!loading && showTasks && items.length">
...
const showTasks = ref(false)   // 工程（詳細）は既定で折りたたみ
...
// 工程表PDF: 非公開バケット → edge(site-attachment-url)で短TTL署名URL（sites/[id].vue と同じ経路）
...
async function load() {
  ...
  if (!error) {
    items.value = (data?.items ?? []) as ProcessItem[]
...
// 工程（詳細）を開いた時も当日位置へ
watch(showTasks, async (v) => { if (v) { await nextTick(); scrollToToday() } })
...
.detail-toggle { width: 100%; background: #fff; border: 1px solid #eee; border-radius: 10px; padding: 10px 12px; font-size: 13px; font-weight: 700; color: #444; text-align: left; margin-bottom: 8px; }
.detail-count { margin-left: 6px; font-size: 11px; font-weight: 600; color: #666; background: #e5e7eb; border-radius: 10px; padding: 0 6px; }
