// ⚠️ AUTO-GENERATED from shared/worker-hours.ts — DO NOT EDIT.
// 共有ロジックの正本は shared/worker-hours.ts。編集したら `npm run sync:shared` で本ファイルを再生成すること。

// ============================================================
//  shared/worker-hours.ts
//  稼働時間の料率別計算・休憩の実効計算・人件費。
//
//  ★正典。LIFF / admin がここを使う（scripts/sync-shared.mjs で配布）。
//   2026-08-16 まで apps/liff/utils と apps/admin/src/lib に2本あり、
//   **同じ関数名で返り値の意味が違っていた**:
//     - liff : workedMin = 前現場を含む累積
//     - admin: workedMin = その現場ぶんだけ（ownWorkedMin）
//   admin の laborBreakdownForReport は `acc += h.workedMin` と自分で累積するので、
//   累積値を返されると二重に足される。＝**admin 版が正**。
//   liff は workedMin を使っていなかったため露見していなかっただけ。
//
//  ★getRateLines は各アプリ側に置く（共有しない）。
//   ラベルの出し方が違う（liff は i18n の gt()、admin は日本語直書き）。
//   ここを無理に共有すると i18n を shared に持ち込むことになる。
// ============================================================
